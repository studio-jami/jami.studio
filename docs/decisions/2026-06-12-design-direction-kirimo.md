...
**Synk is not discarded** — its systematized lattice is earmarked as the **per-product page template**
(Harness, UI Registry, Orchestra, Intercal, Collectiva), where dense, extensible feature communication and
the micro-UI cards genuinely shine. Hub = Kirimo (identity); product pages = Synk system (substance).
Synk's build remains on `origin/design/synk-2` for reference.
...
2. **`/projects` and `/projects/[slug]`.** The project pages need more structure and detail — surface the
   sections with more depth than the home's editorial pacing provides. This is the largest open design item.
...
3. Build out `/projects/[slug]` structure/detail (item 2 above).
4. Adopt the **Synk system** as the per-product page template.
5. Return to the **pre-design product thread**: finish the public site and route all project CTAs through
